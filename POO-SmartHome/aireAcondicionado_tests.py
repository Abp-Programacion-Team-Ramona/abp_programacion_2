...
class TestAireAcondicionadoBasico(unittest.TestCase):
    ...
    def test_set_temperatura_valida(self):
        self.aire.setTemperatura(20)
        self.assertEqual(self.aire.get_temperatura(), 20)
    ...
    def test_set_velocidad_invalida(self):
        for v in (0, 4, -1):
            with self.subTest(v=v):
                with self.assertRaises(ValueError):
                    self.aire.setVelocidad(v)

   
    def test_set_modo_valido_normaliza_minusculas(self):
